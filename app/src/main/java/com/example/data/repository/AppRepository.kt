...
/**
 * AppRepository abstracts data operations for trips, travel agents, package offerings,
 * DIY bookings, wishlist items, chat interactions, and AI-powered trip generation.
 */
open class AppRepository(
    protected val dao: TravyDao,
    protected val geminiSource: GeminiRemoteDataSource = GeminiRemoteDataSource()
) {
    // --- TRIP & PACKAGE OPERATIONS ---

    open fun getPackages(): List<AgentPackage> {
        return mockPackages
    }

    open fun getPackageById(id: String): AgentPackage? {
        return mockPackages.find { it.id == id }
    }

    open fun getPackagesForAgent(agentId: String): List<AgentPackage> {
        return mockPackages.filter { it.agentId == agentId }
    }

    open fun searchPackages(
        query: String = "",
        ...
        return mockPackages.filter { pkg ->
            val matchesQuery = query.isBlank() ||
                    ...
                    pkg.destination.contains(query, ignoreCase = true) ||
                    pkg.agentName.contains(query, ignoreCase = true)
            val matchesCategory = category.isNullOrBlank() ||
                    category.equals("All", ignoreCase = true) ||
                    pkg.category.equals(category, ignoreCase = true)
            val matchesPrice = maxPrice == null || pkg.pricePerPerson <= maxPrice
            matchesQuery && matchesCategory && matchesPrice
        }
    }

    open fun getFeaturedTrips(): List<AgentPackage> {
        return mockPackages.sortedByDescending { it.rating }
    }

    // --- AGENT DATA OPERATIONS ---

    open fun getAgents(): List<TravelAgent> {
        return mockAgents
    }

    open fun getAgentById(id: String): TravelAgent? {
        return mockAgents.find { it.id == id }
    }

    open fun searchAgents(query: String, region: String? = null): List<TravelAgent> {
        return mockAgents.filter { agent ->
            val matchesQuery = query.isBlank() ||
                    agent.name.contains(query, ignoreCase = true) ||
                    agent.tagline.contains(query, ignoreCase = true) ||
                    agent.location.contains(query, ignoreCase = true)
            val matchesRegion = region.isNullOrBlank() ||
                    agent.specializedRegions.any { it.contains(region, ignoreCase = true) }
            matchesQuery && matchesRegion
        }
    }

    // --- DIY TRAVEL SERVICES (FLIGHTS, HOTELS, CABS, TRAINS) ---

    open fun getFlights(): List<FlightOption> = mockFlightOptions
    open fun getFlightOptions(): List<FlightOption> = getFlights()

    open fun getHotels(): List<HotelOption> = mockHotelOptions
    open fun getHotelOptions(): List<HotelOption> = getHotels()

    open fun getCabs(): List<CabOption> = mockCabOptions
    open fun getCabOptions(): List<CabOption> = getCabs()

    open fun getTrains(): List<TrainOption> = mockTrainOptions
    open fun getTrainOptions(): List<TrainOption> = getTrains()

    // --- AI TRIP GENERATION ---

    open suspend fun planTripWithAi(prompt: String): Result<AiTripPlan> {
        return runCatching { geminiSource.generateItinerary(prompt) }
    }

    // --- LOCAL ROOM DB READ/WRITE (CART, WISHLIST, CHAT) ---

    open fun getCartItems(): Flow<List<CartItemEntity>> = dao.getAllCartItems()

    open suspend fun addToCart(
        itemType: String,
        title: String,
        subtitle: String,
    ...
    open suspend fun deleteCartItem(id: Long) {
        dao.deleteCartItem(id)
    }

    open suspend fun markAsBooked(id: Long) {
        dao.markAsBooked(id)
    }

    open suspend fun clearCart() {
        dao.clearCart()
    }

    open fun getWishlist(): Flow<List<WishlistEntity>> = dao.getAllWishlist()

    open suspend fun isWishlisted(id: String): Boolean = dao.isWishlisted(id)

    open suspend fun toggleWishlist(
        id: String,
        itemType: String,
        title: String,
        subtitle: String,
    ...
    open fun getChatMessages(agentId: String): Flow<List<ChatMessageEntity>> =
        dao.getMessagesForAgent(agentId)
    ...
    open suspend fun approveEscrow(messageId: Long) {
        dao.markEscrowPaid(messageId)
    }

    // --- SOCIAL POSTS ---

    open fun getSocialPosts(): List<SocialPost> = mockSocialPosts

    open fun getPostById(id: String): SocialPost? = mockSocialPosts.find { it.id == id }
